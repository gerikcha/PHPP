...
def sol_rad_tilt_surf(weather_data, surface_orientation, albedo):
    ...
    # [Th-CE 2005] §11.2.1.1
    dir_rad = (weather_data["irradiance_direct"] * 1000) * np.cos(theta)
    dir_rad[dir_rad < 0] = 0

    # Diffuse radiation on a wall
    # [Th-CE 2005] §11.2.1.2, Eq. 79, p. 31
    dif_rad = (weather_data["irradiance_diffuse"] * 1000) * (1 + np.cos(B)) / 2
    ...
    dir_h_rad = weather_data["irradiance_direct"] * np.sin(gamma)
    # total reflected radiation
    ref_rad = (dir_h_rad + weather_data["irradiance_diffuse"]) * albedo
